...
export async function getServerSideProps(context) {
  ...
  const getMockData = () => {
    const filtered = MockSearchResults.filter(
        (result) =>
            (result.queries?.request[0]?.searchType || "") ===
            (context.query?.type || "")
    );
  ...
  const getRealData = async () =>
      await fetch(
          ...
          }&cx=${process.env.CONTEXT_KEY}&q=${context.query.q}&${
              context.query.type && "searchType=" + context.query.type
          }&start=${context.query.start || 1}`
      ).then((res) => res.json());
